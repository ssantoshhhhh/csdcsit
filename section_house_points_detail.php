...
while ($student = mysqli_fetch_assoc($students_result)) {
    ...
    // Get detailed appreciations data
    ...
    ";
    $appreciations_detail_stmt = mysqli_prepare($conn, $appreciations_detail_query);
    mysqli_stmt_bind_param($appreciations_detail_stmt, "s", $student['student_id']);
    mysqli_stmt_execute($appreciations_detail_stmt);
    $appreciations_detail_result = mysqli_stmt_get_result($appreciations_detail_stmt);
    $appreciations_details = mysqli_fetch_all($appreciations_detail_result, MYSQLI_ASSOC);

    // Get detailed penalties data
    ...
    ";
    $penalties_detail_stmt = mysqli_prepare($conn, $penalties_detail_query);
    mysqli_stmt_bind_param($penalties_detail_stmt, "s", $student['student_id']);
    mysqli_stmt_execute($penalties_detail_stmt);
    $penalties_detail_result = mysqli_stmt_get_result($penalties_detail_stmt);
    $penalties_details = mysqli_fetch_all($penalties_detail_result, MYSQLI_ASSOC);
                                    ...
                                    <?php foreach ($students as $student): ?>
                                        <tr>
                                            ...
                                            <td>
                                                        ...
                                                        <?php foreach ($student['appreciations_details'] as $appreciation): ?>
                                                            <div class="mb-1">
                                                                <small class="text-success">
                                                                    <i class="fas fa-star"></i> <?php echo htmlspecialchars($appreciation['event_title']); ?>
                                                ...
                                                <?php else: ?>
                                                    <span class="text-muted">No appreciations</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
